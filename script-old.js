...
function updateAdminUI() {
  ...
  if (isLoggedIn) {
    ...
    if (menuServidores) menuServidores.classList.remove('hidden');
    if (menuServidores) menuServidores.classList.add('flex');
  } else {
    ...
    if (menuServidores) menuServidores.classList.add('hidden');
    if (menuServidores) menuServidores.classList.remove('flex');
    
    // Se estiver na aba Servidores e deslogar, joga para Portarias
    if (menuServidores && menuServidores.classList.contains('active')) {
      document.querySelector('[data-tab="portarias"]').click();
    }
  }
}
...
window.openModalPortaria = function(portaria = null) {
  if (!isLoggedIn) {
    showToast('Acesso negado. Faça login.', 'warn');
    return;
  }
...
// ==========================================
// GESTÃO DE SERVIDORES (EDIÇÃO E EXCLUSÃO)
// ==========================================
...
window.deleteServidor = async function(id) {
  ...
  if(confirm("ATENÇÃO: Tem certeza que deseja excluir este servidor? Ele aparecerá como 'Removido' nas portarias.")) {
    try {
...
document.getElementById('form-servidor').addEventListener('submit', async (e) => {
  ...
  try {
    if (editingServidor) {
      await updateDoc(doc(db, "servidores", editingServidor.__backendId), data);
...
window.renderRelatorios = function() {
  // 1. Relatório de Servidores (AGORA EM LISTA E COM BUSCA)
  ...
  if (srvDiv) {
    ...
    if (srvFiltrados.length === 0) { 
      ...
      srvDiv.innerHTML = srvFiltrados.map(s => `
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white border border-slate-200 rounded-xl p-4 shadow-sm hover:border-slate-300 transition-colors">
          <div class="flex-1 min-w-0">
...
// ==========================================
// RECOLHER MENU LATERAL E EVENTOS
// ==========================================
...
// Novo Listener para Busca no Relatório de Servidores
document.getElementById('search-rel-srv')?.addEventListener('input', (e) => {
  searchRelSrvQuery = e.target.value; 
  renderRelatorios();
});
