...
// ==========================================
// JANELA DE DETALHES (Usada pelo Relatório)
// ==========================================
...
// ==========================================
// RELATÓRIOS (ACORDEÃO COM HISTÓRICO DE REVOGADAS)
// ==========================================
...
window.renderRelatorios = function() {
  ...
  portarias.forEach(p => {
    // Mantemos as contas das badges apenas para as VIGENTES (carga de trabalho ativa)
    if (p.status === 'revogada') return; 
    const binding = JSON.parse(p.servidores || '{}');
  ...
  if (srvDiv) {
    ...
    if (srvFiltrados.length === 0) { 
      ...
      srvDiv.innerHTML = srvFiltrados.map(s => {
        // Pega TODO o histórico do servidor (Vigentes e Revogadas)
        ...
        const activeHtml = activePorts.length > 0 
          ? activePorts.map(p => `<div class="text-xs text-slate-600 py-2 border-b border-slate-100 last:border-0 hover:bg-slate-50 transition-colors px-2 rounded"><strong class="text-slate-800">Nº ${p.numero}</strong> - ${p.descricao}</div>`).join('')
          : '<div class="text-xs text-slate-400 py-2 italic px-2">Nenhuma portaria ativa</div>';
          
        // HTML das Revogadas (Histórico)
        const revokedHtml = revokedPorts.length > 0
          ? `<p class="text-[10px] font-bold text-red-400 uppercase tracking-wider mt-4 mb-1 px-2">Histórico: Portarias Revogadas</p>` + 
            revokedPorts.map(p => `<div class="text-xs text-slate-500 py-2 border-b border-slate-100 last:border-0 hover:bg-red-50 transition-colors px-2 rounded bg-slate-50 opacity-80"><strong class="text-slate-700">Nº ${p.numero}</strong> <span class="bg-red-100 text-red-600 px-1.5 py-0.5 rounded text-[9px] font-bold ml-1 uppercase">Revogada</span><br/><span class="line-clamp-1 mt-0.5">${p.descricao}</span></div>`).join('')
          : '';
